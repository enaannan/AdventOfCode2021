...
object main extends App {
  ...
  val rawInput =
    """87,12,53,23,31,70,37,79,95,16,72,9,98,92,5,74,17,60,96,80,75,11,73,33,3,84,81,2,97,93,59,13,77,52,69,83,51,64,48,82,7,49,20,8,36,66,19,0,99,41,91,78,42,40,62,63,57,39,55,47,29,50,58,34,27,43,30,35,22,28,4,14,26,32,10,88,46,65,90,76,38,6,71,67,44,68,86,25,21,24,56,94,18,89,61,15,1,45,54,85
      ...
      |46 29 51 23 99
      |11 60 66 12 52
      |53 87 41 90 49""".stripMargin.stripMargin.split("\\n")
...
//  Part Two:- which board will win last
  ...
  val mutableBoard: ArrayBuffer[Array[Array[Int]]] = ArrayBuffer()


    def lastWinningBoard: ArrayBuffer[Array[Array[Int]]]={

boards.map{board =>
mutableBoard.addOne(board)
}
      def removeWinningBoard(winningBoard:Int)={
        checkers.remove(winningBoard)
        mutableBoard.remove(winningBoard)
      }
      while(checkers.length>1){
      val winningBoard =  findWinner(randomNumbers,mutableBoard.toArray)
        removeWinningBoard(winningBoard)
      }
      findWinner(randomNumbers,mutableBoard.toArray)
mutableBoard
    }

  val b = lastWinningBoard
  val bb = sumOfUnmarkedNumbers(b(0),checkers(0))
println(bb * winningRandomNumber)
}
...
val rawInput = """ec cabfe afebd dbagef afbcg feabcd cdef eafdcbg ecb caegdb | ceb ecb febac ec
                 ...
                 |befcag cefdga ebaf ba bgfcdae fcaeg ecbdg cab fdbacg aecgb | bafe cfgbda geacb acebg""".stripMargin.split("\\n")
...
val input: Array[(Seq[String], Seq[String])] = rawInput.map { line =>
  val tempLine = line.trim.split(" \\| ")
  (tempLine(0).split("\\s"), tempLine(1).split("\\s"))
}

def decode(line: (Seq[String],Seq[String]))={

  val input = line._1
  val output = line._2
  val mappings = mutable.HashMap[Int,String]()

  def decodeOutput(output: Seq[String])={
    val reverseMappings = for ((k, v) <- mappings) yield (v.sorted, k)
    val temp = output.map{out=>
      reverseMappings(out.sorted)
    }
    temp.mkString.toLong
  }


  //finding two seg
  var temp = input.filter(_.length == 2).head
  if(temp != null){
    mappings.addOne(1,temp)
  }

  //finding three seg
  temp = input.filter(_.length == 3).head
  if(temp != null){
    mappings.addOne(7,temp)
  }

  //finding four seg
  temp = input.filter(_.length == 4).head
  if(temp != null){
    mappings.addOne(4,temp)
  }

  //finding eight seg
  temp = input.filter(_.length == 7).head
  if(temp != null){
    mappings.addOne(8,temp)
  }

  //finding 6 seg
  val sixSeg = input.filter(_.length == 6)

  //all of 4 is in 9 but the other 6 seg do not conform to this
  temp = sixSeg.filter{ seg =>
    mappings(4).map { c => seg.contains(c) }.forall(y => y)
  }.head
  if(temp != null){
    mappings.addOne(9,temp)
  }

  // all of 1 matches zero
  temp = sixSeg.filter(_ != mappings(9)).filter{ seg =>
    mappings(1).map { c => seg.contains(c) }.forall(y => y)
  }.head
  if(temp != null){
    mappings.addOne(0,temp)
  }

  //rem is 6
  temp =  sixSeg.filter(a => a != mappings(0) && a != mappings(9) ).head
  if(temp != null){
    mappings.addOne(6,temp)
  }

  //    finding 5 seg
  val fiveSeg = input.filter(_.length == 5)

  //all one in 3
  temp = fiveSeg.filter{ seg =>
    mappings(1).map { c => seg.contains(c) }.forall(y => y)
  }.head
  if(temp != null){
    mappings.addOne(3,temp)
  }

  //all five in 6 - rem 3
  temp = fiveSeg.filter(_!= mappings(3)).filter{ seg =>
    seg.map{c => mappings(6).contains(c)}.forall(y=>y)
  }.head
  if(temp != null){
    mappings.addOne(5,temp)
  }

  //rem is 2
  temp = fiveSeg.filter(c=> c!= mappings(3) && c!= mappings(5)).head
  if(temp != null){
    mappings.addOne(2,temp)
  }

  decodeOutput(output)
}

val k = input.map(decode).sum
println(k)